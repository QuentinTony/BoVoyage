...
public class PrestationController {
	...
	public @ResponseBody void imageDestination(@PathVariable("idD") int id, HttpServletResponse response) {
		...
		byte[] image = (prService.getPrestation(id)).getPhoto();
	...
	public String submitUpdate(@ModelAttribute("prestation") Prestation ve, RedirectAttributes ra, MultipartFile file) {
		try {
			byte[] photo1 = file.getBytes();
			ve.setPhoto((byte[]) photo1);
		} catch (IOException e) {
